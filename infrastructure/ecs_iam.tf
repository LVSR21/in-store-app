...
###############################################################
## Create IAM Policy for ECS Tasks to access Secrets Manager ##
###############################################################

resource "aws_iam_policy" "ecs_task_secrets_policy" {
  name        = "${var.namespace}_ECS_TaskSecretsPolicy_${var.environment}" # Name of the IAM Policy
  description = "Allows ECS tasks to access Secrets Manager"                # Description of the IAM Policy

  policy = jsonencode({                                                     # The policy document for the IAM policy. The policy document is a JSON document that defines the permissions that the policy allows.
    Version = "2012-10-17",                                                 # The version of the policy document.
    Statement = [{                                                          # The statement block defines the permissions that the policy allows. In this case, the policy allows the IAM role to access Secrets Manager.
      Effect = "Allow",                                                     # The effect that the policy has. In this case, the effect is "Allow", which allows the action to be performed.
      Action = [                                                            # The actions that the policy allows. In this case, the policy allows the IAM role to perform various actions related to Secrets Manager.
        "secretsmanager:GetSecretValue",                                    # The actions that the policy allows. In this case, the policy allows the IAM role to get the value of a secret.
        "secretsmanager:DescribeSecret"                                     # The actions that the policy allows. In this case, the policy allows the IAM role to describe a secret.
      ],
      Resource = aws_secretsmanager_secret.mongodb.arn                      # The resources that the policy allows the IAM role to access. In this case, the policy allows the IAM role to access the MongoDB secret stored in Secrets Manager.
    }]
  })
}
...
##############################################################
## Create IAM Role Policy Attachment for ECS Task Execution ##
##############################################################

resource "aws_iam_role_policy_attachment" "ecs_task_execution_role_secrets_policy" {
  role       = aws_iam_role.ecs_task_execution_role.name  # Name of the IAM Role
  policy_arn = aws_iam_policy.ecs_task_secrets_policy.arn # ARN of the policy to attach - this specific ARN references the IAM Policy for ECS Tasks to access Secrets Manager.
}
